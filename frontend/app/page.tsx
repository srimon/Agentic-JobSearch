...
export default function Home(){
 ...
 useEffect(()=>{const params=new URLSearchParams(window.location.search);if(['matches','emailed','archive','saved','observability','quality','model'].includes(params.get('view')||'')){setTab(params.get('view')!);setDate(params.get('view')==='archive'?'any':'24h');setShowDismissed(params.get('view')==='emailed')}},[]);
 ...
 useEffect(()=>{if(!user)return;if(['intake','applications','observability','quality','model'].includes(tab)){setLoading(false);return;}let active=true;setLoading(true);setError('');const params=new URLSearchParams({q:query,date,level,mode,page:String(page),sort,view:tab,show_dismissed:String(showDismissed)});
 ...
 const labels:Record<string,string>={archive:'Archive',emailed:'Emailed jobs',matches:'All opportunities',saved:'Saved jobs',review:'Needs review',sources:'Source coverage',runs:'Collection runs',intake:'Resume & profile',applications:'Application checks',observability:'Observability',quality:'Data quality',model:'Data model'};
 ...
 {user&&!['observability','quality','model'].includes(tab)&&<CollectionStatus version={version} date={date}/>}
