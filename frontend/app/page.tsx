...
export default function Home(){
 ...
 const adminOnly=['workflow','sources','runs','review','observability','quality','model','governance'];
 useEffect(()=>{if(user&&!admin&&(adminOnly.includes(tab)||tab.startsWith('data-')))choose('matches')},[user,admin,tab]);
 useEffect(()=>{if(!user)return;if(!admin&&adminOnly.includes(tab))return;if(tab.startsWith('data-')||['workflow','intake','applications','observability','quality','model','governance','account'].includes(tab)){setLoading(false);return;}let active=true;const seq=++listSeq.current;setLoading(true);setError('');const params=listParams();
 ...
 task.catch(e=>{if(active)setError(e.message)}).finally(()=>{if(active)setLoading(false)});return()=>{active=false};},[user,tab,query,date,level,mode,page,version,showDismissed,sort,usState,titleFamily]);
 ...
 return <HubLinksContext.Provider value={{links,operator:admin}}><div className="brand-page"><BrandHeader user={user} hub={links.hub} signingOut={signingOut} onSignOut={signOut} ready={!!session}/><div className="shell"><MotionScene loading={loading} saving={!!busyJob} label={labels[tab]}/><aside className="sidebar">{links.hub&&<a className="hub-link" href={links.hub}><ArrowLeft size={16}/>Products</a>}<div className="workspace-label">YOUR WORKSPACE</div><nav aria-label="Main navigation">

 {[['matches','Opportunities',BriefcaseBusiness],['saved','Saved jobs',Bookmark],['emailed','Emailed jobs',Mail],['archive','Archive',Archive],...(member?[['intake','Resume & profile',ShieldCheck],['applications','Application checks',BriefcaseBusiness]]:[]),...(admin?[['review','Needs review',ShieldCheck],['workflow','Daily workflow',Clock3],['sources','Sources',Database],['runs','Activity',Activity],['observability','Observability',Activity]]:[]),...(user?[['account','Account',UserRound]]:[])].map(([key,label,Icon])=>{const C=Icon as typeof Search;return <button key={key as string} aria-current={tab===key?'page':undefined} className={tab===key?'nav-item active':'nav-item'} onClick={()=>choose(key as string)}><C size={18}/>{label as string}{key==='matches'&&<span className="nav-dot"/>}</button>})}{links.prep&&<a className="nav-item" href={links.prep} target="_blank" rel="noopener noreferrer"><BriefcaseBusiness size={18}/>Job Prep<ArrowUpRight size={15}/></a>}{admin&&dataManagementEnabled&&<DataNavigation tab={tab} choose={choose}/>}</nav>
 ...
 {user&&admin&&!tab.startsWith('data-')&&!['workflow','observability','quality','model','governance','account'].includes(tab)&&<CollectionStatus version={version} date={date}/>}
 ...
 <><div className="filters"><label>Order<select aria-label="Job order" value={sort} onChange={e=>{setSort(e.target.value);setPage(1)}}><option value="recommended">Recommended for you</option><option value="newest">Newest first</option></select></label><div className="search"><Search size={19}/><input value={q} onChange={e=>setQ(e.target.value)} placeholder="Search title or company" aria-label="Search title or company"/></div><label>Date posted<select value={date} onChange={e=>{setDate(e.target.value);setPage(1)}}><option value="any">Any time</option><option value="24h">Past 24 hours</option><option value="7d">Past 7 days</option><option value="30d">Past 30 days</option></select></label><label>Level<select value={level} onChange={e=>{setLevel(e.target.value);setPage(1)}}><option value="">All levels</option>{['Director','Senior Director','VP','SVP','C-suite'].map(x=><option key={x}>{x}</option>)}</select></label><label>Workplace<select value={mode} onChange={e=>{setMode(e.target.value);setPage(1)}}><option value="">All workplaces</option>{['Remote','Hybrid','On-site','Unknown'].map(x=><option key={x}>{x}</option>)}</select></label><label>State<select aria-label="State" value={usState} onChange={e=>{setUsState(e.target.value);setPage(1)}}><option value="">All states</option>{facets.states.map(f=><option key={f.value} value={f.value}>{f.label} ({f.count})</option>)}</select></label><label>Job title<select aria-label="Job title" value={titleFamily} onChange={e=>{setTitleFamily(e.target.value);setPage(1)}}><option value="">All job titles</option>{facets.titles.map(f=><option key={f.value} value={f.value}>{f.label} ({f.count})</option>)}</select></label></div>
