...
export default function Home(){
 ...
 {[['matches','Opportunities',BriefcaseBusiness],['saved','Saved jobs',Bookmark],['emailed','Emailed jobs',Mail],['archive','Archive',Archive],...(member?[['intake','Resume & profile',ShieldCheck],['applications','Application checks',BriefcaseBusiness]]:[]),...(admin?[['review','Needs review',ShieldCheck],['workflow','Daily workflow',Clock3],['sources','Sources',Database],['runs','Activity',Activity],['observability','Observability',Activity],['admin','Admin console',Gauge]]:[]),...(user?[['account','Account',UserRound]]:[])].map(([key,label,Icon])=>{const C=Icon as typeof Search;return <button key={key as string} aria-current={tab===key?'page':undefined} className={tab===key?'nav-item active':'nav-item'} onClick={()=>choose(key as string)}><C size={18}/>{label as string}{key==='matches'&&<span className="nav-dot"/>}</button>})}{links.prep&&<a className="nav-item" href={links.prep} target="_blank" rel="noopener noreferrer"><BriefcaseBusiness size={18}/>Job Prep<ArrowUpRight size={15}/></a>}{admin&&dataManagementEnabled&&<DataNavigation tab={tab} choose={choose}/>}</nav>
