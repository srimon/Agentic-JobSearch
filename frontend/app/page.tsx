...
export default function Home(){
 ...
 useEffect(()=>{const params=new URLSearchParams(window.location.search);if(['matches','emailed','archive','saved','observability','quality'].includes(params.get('view')||'')){setTab(params.get('view')!);setDate(params.get('view')==='archive'?'any':'24h');setShowDismissed(params.get('view')==='emailed')}},[]);
 ...
 useEffect(()=>{if(!user)return;if(['intake','applications','observability','quality'].includes(tab)){setLoading(false);return;}let active=true;setLoading(true);setError('');const params=new URLSearchParams({q:query,date,level,mode,page:String(page),sort,view:tab,show_dismissed:String(showDismissed)});
 ...
 const labels:Record<string,string>={archive:'Archive',emailed:'Emailed jobs',matches:'All opportunities',saved:'Saved jobs',review:'Needs review',sources:'Source coverage',runs:'Collection runs',intake:'Resume & profile',applications:'Application checks',observability:'Observability',quality:'Data quality'};
 ...
 {[['matches','Opportunities',BriefcaseBusiness],['saved','Saved jobs',Bookmark],['emailed','Emailed jobs',Mail],['archive','Archive',Archive],...(member?[['intake','Resume & profile',ShieldCheck],['applications','Application checks',BriefcaseBusiness]]:[]),...(operator?[['review','Needs review',ShieldCheck]]:[]),['sources','Sources',Database],...(operator?[['runs','Activity',Activity],['observability','Observability',Activity],...(process.env.NEXT_PUBLIC_JOBSEARCH_ENVIRONMENT==='staging'?[['quality','Data quality',ShieldCheck]]:[])]:[])].map(([key,label,Icon])=>{const C=Icon as typeof Search;return <button key={key as string} aria-current={tab===key?'page':undefined} className={tab===key?'nav-item active':'nav-item'} onClick={()=>choose(key as string)}><C size={18}/>{label as string}{key==='matches'&&<span className="nav-dot"/>}</button>})}</nav>
 ...
 {user&&!['observability','quality'].includes(tab)&&<CollectionStatus version={version} date={date}/>}
