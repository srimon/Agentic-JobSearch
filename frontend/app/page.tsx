...
export default function Home(){
 ...
 <><div className="filters"><label><span className="filter-name"><MotionIcon name="sliders" motion="sway" small/>Order</span><select aria-label="Job order" value={sort} onChange={e=>{setSort(e.target.value);setPage(1)}}><option value="recommended">Recommended for you</option><option value="newest">Newest first</option></select></label><div className="search"><MotionIcon name="search" motion="orbit" small/><input value={q} onChange={e=>setQ(e.target.value)} placeholder="Search title or company" aria-label="Search title or company"/></div><label><span className="filter-name"><MotionIcon name="calendar" motion="bounce" small/>Date posted</span><select value={date} onChange={e=>{setDate(e.target.value);setPage(1)}}><option value="any">Any time</option><option value="24h">Past 24 hours</option><option value="7d">Past 7 days</option><option value="30d">Past 30 days</option></select></label><label><span className="filter-name"><MotionIcon name="layers" motion="pulse" small/>Level</span><select value={level} onChange={e=>{setLevel(e.target.value);setPage(1)}}><option value="">All levels</option>{['Director','Senior Director','VP','SVP','C-suite'].map(x=><option key={x}>{x}</option>)}</select></label><label><span className="filter-name"><MotionIcon name="map" motion="wiggle" small/>Workplace</span><select value={mode} onChange={e=>{setMode(e.target.value);setPage(1)}}><option value="">All workplaces</option>{['Remote','Hybrid','On-site','Unknown'].map(x=><option key={x}>{x}</option>)}</select></label><label><span className="filter-name"><MotionIcon name="globe" motion="spin" small/>State</span><select aria-label="State" value={usState} onChange={e=>{setUsState(e.target.value);setPage(1)}}><option value="">All states</option>{facets.states.map(f=><option key={f.value} value={f.value}>{f.label} ({f.count})</option>)}</select></label><label><span className="filter-name"><MotionIcon name="tag" motion="nudge" small/>Job title</span><select aria-label="Job title" value={titleFamily} onChange={e=>{setTitleFamily(e.target.value);setPage(1)}}><option value="">All job titles</option>{facets.titles.map(f=><option key={f.value} value={f.value}>{f.label} ({f.count})</option>)}</select></label></div>

 <label className="dismissed-filter"><input type="checkbox" checked={showDismissed} onChange={e=>{setShowDismissed(e.target.checked);setPage(1)}}/><MotionIcon name="archive" motion="wiggle" small/> Show dismissed</label>
