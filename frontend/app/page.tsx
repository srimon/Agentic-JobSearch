...
export default function Home(){
 ...
 useEffect(()=>{if(!user)return;if(tab.startsWith('data-')||['workflow','intake','applications','observability','quality','model','governance'].includes(tab)){setLoading(false);return;}let active=true;setLoading(true);setError('');const params=new URLSearchParams({q:query,date,level,mode,page:String(page),sort,view:tab,show_dismissed:String(showDismissed)});
 ...
 {[['matches','Opportunities',BriefcaseBusiness],['saved','Saved jobs',Bookmark],['emailed','Emailed jobs',Mail],['archive','Archive',Archive],...(member?[['intake','Resume & profile',ShieldCheck],['applications','Application checks',BriefcaseBusiness]]:[]),...(operator?[['review','Needs review',ShieldCheck]]:[]),['workflow','Daily workflow',Clock3],['sources','Sources',Database],...(operator?[['runs','Activity',Activity],['observability','Observability',Activity],]:[])].map(([key,label,Icon])=>{const C=Icon as typeof Search;return <button key={key as string} aria-current={tab===key?'page':undefined} className={tab===key?'nav-item active':'nav-item'} onClick={()=>choose(key as string)}><C size={18}/>{label as string}{key==='matches'&&<span className="nav-dot"/>}</button>})}{operator&&dataManagementEnabled&&<DataNavigation tab={tab} choose={choose}/>}</nav>

 <a className="nav-item" href="http://localhost:3188/" target="_blank" rel="noopener noreferrer"><BriefcaseBusiness size={18}/>Training &amp; Preparation<ArrowUpRight size={15}/></a>
 ...
 {user&&!tab.startsWith('data-')&&!['workflow','observability','quality','model','governance'].includes(tab)&&<CollectionStatus version={version} date={date}/>}
