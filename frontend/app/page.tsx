...
export default function Home(){
 ...
 return <HubLinksContext.Provider value={{links,operator}}><div className="shell"><MotionScene loading={loading} saving={!!busyJob} label={labels[tab]}/><aside className="sidebar">{links.hub&&<a className="hub-link" href={links.hub}><ArrowLeft size={16}/>Bagala</a>}<div className="workspace-label">YOUR WORKSPACE</div><nav aria-label="Main navigation">

 {[['matches','Opportunities',BriefcaseBusiness],['saved','Saved jobs',Bookmark],['emailed','Emailed jobs',Mail],['archive','Archive',Archive],...(member?[['intake','Resume & profile',ShieldCheck],['applications','Application checks',BriefcaseBusiness]]:[]),...(operator?[['review','Needs review',ShieldCheck]]:[]),['workflow','Daily workflow',Clock3],['sources','Sources',Database],...(operator?[['runs','Activity',Activity],['observability','Observability',Activity],]:[]),...(user?[['account','Account',UserRound]]:[])].map(([key,label,Icon])=>{const C=Icon as typeof Search;return <button key={key as string} aria-current={tab===key?'page':undefined} className={tab===key?'nav-item active':'nav-item'} onClick={()=>choose(key as string)}><C size={18}/>{label as string}{key==='matches'&&<span className="nav-dot"/>}</button>})}{links.prep&&<a className="nav-item" href={links.prep} target="_blank" rel="noopener noreferrer"><BriefcaseBusiness size={18}/>Training &amp; Preparation<ArrowUpRight size={15}/></a>}{operator&&dataManagementEnabled&&<DataNavigation tab={tab} choose={choose}/>}</nav>
 ...
 {user&&!tab.startsWith('data-')&&!['workflow','observability','quality','model','governance','account'].includes(tab)&&<CollectionStatus version={version} date={date}/>}
 ...
 <footer>Bagala · Job Search <span>Original sources. Clear evidence. Your decision.</span></footer></section></main>
