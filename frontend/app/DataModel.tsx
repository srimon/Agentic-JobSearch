...
export default function DataModel({version}:{version:number}){
 ...
 useEffect(()=>{const controller=new AbortController();setBusy(true);setError('');setData(null);apiFetch('/data-model',{cache:'no-store',signal:controller.signal}).then(async r=>{if(!r.ok)throw new Error(r.status===403?'Operator access is required.':'Data model is unavailable.');return r.json()}).then(setData).catch(e=>{if(!controller.signal.aborted)setError(e.message)}).finally(()=>{if(!controller.signal.aborted)setBusy(false)});return()=>controller.abort()},[version,refresh]);
 return <section className="dq" aria-label="PostgreSQL data model"><div className="dq-hero"><Network size={38}/><div><span className="eyebrow">POSTGRESQL</span><h2>Explore your database structure</h2><p>SchemaSpy tables, columns and declared relationships. Generated from an empty copy of the production schema.</p></div><button className="secondary" disabled={busy} onClick={()=>setRefresh(n=>n+1)}><RefreshCw size={16} className={busy?'spin':''}/>Reload snapshot</button></div>
