...
export default function DataModel({version}:{version:number}){
 ...
 return <section className="dq" aria-label="PostgreSQL data model"><div className="dq-hero"><Network size={38}/><div><span className="eyebrow">POSTGRESQL · STAGING</span><h2>Explore your database structure</h2><p>SchemaSpy tables, columns and declared relationships. Generated from an empty copy of the staging schema.</p></div><button className="secondary" disabled={busy} onClick={()=>setRefresh(n=>n+1)}><RefreshCw size={16} className={busy?'spin':''}/>Reload snapshot</button></div>
 {error&&<p role="alert" className="message error">{error}</p>}
 {busy&&<p role="status">Loading the latest generated model…</p>}
 {data?.status==='not_generated'&&<p className="empty">No model has been generated yet. Run the staging diagram generator.</p>}
