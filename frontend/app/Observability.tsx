...
const tools = {
 ...
 phoenix:{name:'Phoenix',icon:Activity,description:'Inspect Jobsearch traces, spans and execution timing.',screens:[['GraphQL displays','graphql-displays'],['Projects & traces','projects'],['GraphQL explorer','graphql?q='+encodeURIComponent('query JobsearchProjects { projects(first: 10) { edges { node { id name } } } }')]]}
};
...
export default function Observability({version}:{version:number}){
 ...
 const current=tools[tool];const path=current.screens[screen]?.[1]||current.screens[0][1];const custom=tool==='phoenix'&&path==='graphql-displays';const url=custom?'':'/api/monitoring/'+tool+'/'+path;
 ...
 useEffect(()=>{setLoaded(custom)},[url,custom]);
 ...
 <div className="obs-controls"><div className="obs-screen-buttons">{current.screens.map(([label],index)=><button className={screen===index?'primary':'secondary'} key={label} onClick={()=>{setScreen(index);setReload(n=>n+1)}}>{label}</button>)}</div>{!custom&&<button className="secondary" onClick={()=>setReload(n=>n+1)}><RefreshCw size={16}/>Reload</button>}<button className="secondary" onClick={()=>setExpanded(!expanded)}><Maximize2 size={16}/>{expanded?'Exit expanded view':'Expand'}</button>{!custom&&<a href={url} target="_blank" rel="noreferrer"><ExternalLink size={16}/>Open full view</a>}</div>
 {!custom&&(!loaded||error)&&<p role="status">{error||'Loading '+current.name+'…'}</p>}
 {ready&&custom&&<PhoenixGraphQLDisplays refreshToken={reload+version}/>}
 {ready&&!custom&&<iframe key={url+reload+version} src={url} title={current.name+' live interface'} className="obs-vendor-frame" onLoad={()=>setLoaded(true)} referrerPolicy="no-referrer"/>}
