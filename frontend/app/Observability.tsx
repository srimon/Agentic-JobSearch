...
export default function Observability({version}:{version:number}){
 ...
 return <section className={'observability-panel mbk-monitoring '+(expanded?'obs-expanded':'')} aria-label="Observability tools">
  <header className="monitor-heading"><div><span className="eyebrow eyebrow--admin">Admin</span><h2>{tool==='phoenix'?'Explainability':'Observability'} · Job Search</h2><p>{tool==='phoenix'?'Explore the recorded trace of each operation in Phoenix.':'Grafana dashboards and the Prometheus series they are drawn from.'}</p></div></header>
