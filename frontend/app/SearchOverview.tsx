...
export default function SearchOverview({view}:{view:string}){
 ...
 // `slug` is the stable half of the data-text key: the words may be edited, the key may not (page-text.mjs).
 const steps:{icon:IconName;motion:Motion;slug:string;title:string;text:string}[]=emailed?[
  {icon:'cursor',motion:'nudge',slug:'open',title:'Open the posting',text:'Review the employer’s requirements.'},
  {icon:'check',motion:'pulse',slug:'next',title:'Take the next step',text:'Use the action notes on each job.'},
  {icon:'lock',motion:'wiggle',slug:'archive',title:'Update & archive',text:'A final status locks the emailed listing.'}
 ]:[
  {icon:'search',motion:'orbit',slug:'discover',title:'Discover',text:'New opportunities from trusted employers.'},
  {icon:'check',motion:'pulse',slug:'review',title:'Review',text:'Compare the role with your experience.'},
  {icon:'chart',motion:'bounce',slug:'refine',title:'Refine',text:'Your feedback improves future rankings.'}
 ];
 ...
 {!emailed&&<div className="overview-intro"><div><span className="overview-kicker"><span/><span data-text="jobsearch.overview.kicker">Your search, with direction</span></span><h2 data-text="jobsearch.overview.heading">Find the next place to make an impact.</h2><p data-text="jobsearch.overview.lede">Explore opportunities that fit your experience. Build a more focused shortlist with every decision.</p><span className="private-note"><LockKeyhole size={13}/><span data-text="jobsearch.overview.private-note">Your profile. Your decisions.</span></span></div>
 ...
 <div className="overview-steps">{steps.map(({icon,motion,slug,title,text},i)=><div className="overview-step" key={slug}><span className="step-icon"><MotionIcon name={icon} motion={motion}/></span><div><strong data-text={'jobsearch.overview.'+(emailed?'emailed':'search')+'-'+slug+'-title'}>{title}</strong><p data-text={'jobsearch.overview.'+(emailed?'emailed':'search')+'-'+slug+'-text'}>{text}</p></div>{i<2&&<ArrowRight className="step-arrow" size={16}/>}</div>)}</div>
 </section>
}
