...
export default function MotionScene({loading,saving,label}:{loading:boolean;saving:boolean;label:string}){
 ...
 return <><div className="motion-background" aria-hidden="true"><svg viewBox="0 0 1400 1000" preserveAspectRatio="xMidYMid slice"><defs><radialGradient id="ambientTeal"><stop stopColor="#34d3b5" stopOpacity=".24"/><stop offset="1" stopColor="#34d3b5" stopOpacity="0"/></radialGradient><radialGradient id="ambientBlue"><stop stopColor="#5b8df4" stopOpacity=".2"/><stop offset="1" stopColor="#5b8df4" stopOpacity="0"/></radialGradient></defs><circle className="ambient-orbit" cx="1100" cy="240" r="430" fill="url(#ambientTeal)"/><circle cx="420" cy="730" r="490" fill="url(#ambientBlue)"/><g fill="none" stroke="#5896b7" strokeWidth="1" opacity=".19">{[0,1,2,3,4,5,6,7].map(i=><path key={i} d={`M 250 ${110+i*23} C 550 ${-100+i*28}, 750 ${500+i*20}, 1420 ${130+i*35}`} />)}</g><path className="contour-flow" d="M250 202C550 12 750 570 1420 270" fill="none" stroke="#179fba" strokeWidth="2" strokeDasharray="5 110" opacity=".35"/></svg></div>
