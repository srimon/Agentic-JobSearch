...
export default function Governance(){
 ...
 return <section className="dq" aria-label="Data governance"><div className="dq-hero"><BookOpen size={38}/><div><span className="eyebrow">OPENMETADATA · PRODUCTION</span><h2>Understand and govern your data</h2><p>Explore the catalog, declared data lineage and business definitions.</p></div><button className="secondary" onClick={()=>setRefresh(n=>n+1)}><RefreshCw size={16}/>Reload verification</button></div>
 {error&&<p role="alert" className="message error">{error}</p>}{!data&&!error&&<p>Loading catalog verification…</p>}{data?.status==='not_generated'&&<p>The production catalog has not been verified yet.</p>}
