...
| JOBSEARCH_PUBLIC_ORIGIN (new) | = origin | Browser-facing origin used in emailed links; production `https://jobs.bagala.ai`. |
...
Production values for bagala.ai: `JOBSEARCH_PUBLIC_ORIGIN=https://jobs.bagala.ai`, `JOBSEARCH_ALLOWED_ORIGINS=https://jobs.bagala.ai,http://localhost:3105`, `JOBSEARCH_COOKIE_DOMAIN=bagala.ai`, `JOBSEARCH_HUB_ORIGINS=https://hub.bagala.ai,http://localhost:3180`, `JOBSEARCH_LIBRARY_ORIGINS=https://library.bagala.ai,...`, `JOBSEARCH_SIGNUP_ENABLED=true`, `JOBSEARCH_MAIL_TRANSPORT=resend` with the key mounted at the configured path, and `HTTPS_PROXY` for the mailer pod's egress.
